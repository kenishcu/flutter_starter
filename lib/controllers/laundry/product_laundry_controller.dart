...
class ProductLaundryController extends GetxController with GetTickerProviderStateMixin {

  final GetStorage box;

  final LaundryRepository laundryRepository;

  ProductLaundryController({required this.box, required this.laundryRepository});
  ...
  Future initLaundry() async {

    await initMealTypes();

    var response = await laundryRepository.findAllLaundryCate();
    if(response.status == true) {
      ...
      for (var element in myTabs) {
        ...
        var responseSubCate = await laundryRepository.findAllLaundrySubCate(element.categoryId!);
      ...
      for (var element in myTabs) {
        var responseProduct = await laundryRepository.findAllProduct("", "", element.categoryId!, 50, 1);
  ...
  Future initMealTypes() async {
    ...
    final res = await laundryRepository.getAllMealType();
  ...
  void setSelectedCategory(int selectedCategory) async {
    ...
    var responseProduct = await laundryRepository.findAllProduct(myCategories[selectedTab.value][selectedCategory].categoryId.toString(), "", myTabs[selectedTab.value].categoryId.toString(), 50, 1);
    if(responseProduct.status == true) {
      List<ProductModel> ps = [];
      for (var p in responseProduct.results) {
        ps.add(ProductModel.fromJson(p));
  ...
  Future<bool> order(GlobalKey<AnimatedListState> listKey) async {
    if (itemEs.isNotEmpty) {
      ...
      ResultModel res = await laundryRepository.orderProducts(products);
      if(res.status == true) {
  ...
  Future search(String text) async {
    var responseProduct = await laundryRepository.findAllProduct(myCategories[selectedTab.value][selectedCategory].categoryId.toString(), text, myTabs[selectedTab.value].categoryId.toString(), 50, 1);
    if(responseProduct.status == true) {
      List<ProductModel> ps = [];
      for (var p in responseProduct.results) {
        ps.add(ProductModel.fromJson(p));
  ...
  Future initScreenLaundry() async {
  ...
  Future<List<OrderModel>> getOrderHistory() async {
    ...
    ResultModel res = await laundryRepository.getOrders(patientId, receptionQueueId);
