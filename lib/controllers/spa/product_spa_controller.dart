...
class ProductSpaController extends GetxController with GetTickerProviderStateMixin {

  final GetStorage box;

  final SpaRepository spaRepository;

  ProductSpaController({required this.box, required this.spaRepository});
  ...
  Future initSpa() async {

    await initMealTypes();

    var response = await spaRepository.findAllSpaCate();
    if(response.status == true) {
      ...
      for (var element in myTabs) {
        ...
        var responseSubCate = await spaRepository.findAllSpaSubCate(element.categoryId!);
      ...
      for (var element in myTabs) {
        var responseProduct = await spaRepository.findAllProduct("", "", element.categoryId!, 50, 1);
  ...
  Future initMealTypes() async {
    ...
    final res = await spaRepository.getAllMealType();
  ...
  void setSelectedCategory(int selectedCategory) async {
    ...
    var responseProduct = await spaRepository.findAllProduct(myCategories[selectedTab.value][selectedCategory].categoryId.toString(), "", myTabs[selectedTab.value].categoryId.toString(), 50, 1);
    if(responseProduct.status == true) {
      List<ProductModel> ps = [];
      for (var p in responseProduct.results) {
        ps.add(ProductModel.fromJson(p));
  ...
  Future<bool> order(GlobalKey<AnimatedListState> listKey) async {
    if (itemEs.isNotEmpty) {
      ...
      ResultModel res = await spaRepository.orderProducts(products);
      if(res.status == true) {
  ...
  Future search(String text) async {
    var responseProduct = await spaRepository.findAllProduct(myCategories[selectedTab.value][selectedCategory].categoryId.toString(), text, myTabs[selectedTab.value].categoryId.toString(), 50, 1);
    if(responseProduct.status == true) {
      List<ProductModel> ps = [];
      for (var p in responseProduct.results) {
        ps.add(ProductModel.fromJson(p));
  ...
  Future<List<OrderModel>> getOrderHistory() async {
    ...
    ResultModel res = await spaRepository.getOrders(patientId, receptionQueueId);
